...
if (!empty($_POST)) {
    if (empty($_POST['idusuario']) || empty($_POST['codcliente']) || empty($_POST['producto']) || empty($_POST['cantidad'])) {
        ...
        if ($cantidad > $productos_disponibles[$codproducto]['existencia']) {
            ...
            if ($query_actualizar_existencia) {
                ...
                if ($query_insert_venta) {
                    ...
                    // Insertar el detalle de la venta para el producto seleccionado
                    $query_insert_detalle = mysqli_query($connection, "INSERT INTO detalle_venta (idventa, codproducto, cantidad) VALUES ('$idventa', '$codproducto', '$cantidad')");
                    ...
                    if ($query_insert_detalle) {
                        ...
                        // Rollback la transacción si hay un error en el detalle de la venta
                        mysqli_rollback($connection);
                        $alert = '<p class="msg_error">Error al registrar el detalle de la venta: ' . mysqli_error($connection) . '</p>';
                    }
                } else {
                    // Rollback la transacción si hay un error al insertar la venta
                    mysqli_rollback($connection);
                    $alert = '<p class="msg_error">Error al registrar la venta: ' . mysqli_error($connection) . '</p>';
                }
            } else {
                // Rollback la transacción si hay un error al actualizar la existencia del producto
                mysqli_rollback($connection);
...
// Manejo de la respuesta del servidor para AJAX
if (isset($_POST['ajax'])) {
    echo $alert;
    exit;
}
